				...
				for (int t = 0; t < pt; t++) {
					...
					for (int enemyIndex = 0; enemyIndex < enemies.Length; enemyIndex++) {
						...
						if (seen && enemies [enemyIndex].cells [par.t + t].Length > 0) {
							...
							currentPath.los += f;
							currentPath.los3 += g;
							currentPath.los3Norm += g * (dangerCells [(int)pos2d.x] [(int)pos2d.y] / maxDanger);
						}
					}
				}
				...
				for (int t = 0; t < pt; t++) {
					...
					foreach (Enemy each in enemies) {
						...
						if (astarpath.Count > 0) {
							...
							currentPath.danger += 1 / (l * l);
							currentPath.danger3 += 1 / (l * l * l);
							currentPath.danger3Norm += (1 / (l * l * l)) * (dangerCells [startX] [startY] / maxDanger);
						}
					}
				}
